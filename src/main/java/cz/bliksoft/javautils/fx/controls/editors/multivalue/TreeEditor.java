 ...
 *
 * <p>Node types are defined via {@link ITreeNodeType}. Each type controls what child
 * types are allowed, how nodes are displayed, and whether inline rename is supported.
 * The add button adapts to the selected node's allowed child types (shown as a
 * {@link SplitMenuButton} when multiple child types are available).
 *
 * <p>When the selected node's type provides an inline editor that
...
public class TreeEditor<N> extends VBox {
    ...
    public TreeEditor(Function<N, ITreeNodeType<N>> typeResolver) {
        ...
        treeView.getSelectionModel().selectedItemProperty().addListener((obs, o, sel) -> {
            if (sel == null || sel.getValue() == null) {
                ...
                for (TreeItem<N> ti = sel; ti != null; ti = ti.getParent())
                    if (ti.getValue() != null) path.add(0, ti.getValue());
        ...
        delBtn.setOnAction(e -> {
            ...
            if (parent != null) {
                ...
                if (parentType != null) parentType.removeChild(parent, node);
            }
    ...
    public ReadOnlyObjectProperty<N> selectedItemProperty() { return selectedItem.getReadOnlyProperty(); }
    public N getSelectedItem() { return selectedItem.get(); }

    public ReadOnlyObjectProperty<List<N>> selectedPathProperty() { return selectedPath.getReadOnlyProperty(); }
    public List<N> getSelectedPath() { return selectedPath.get(); }

    public void setSelectedItem(N item) {
        TreeItem<N> ti = itemMap.get(item);
        if (ti != null) treeView.getSelectionModel().select(ti);
    }
    ...
    private void addChild(TreeItem<N> parentItem, ITreeNodeType<N> childType) {
        ...
        ITreeNodeType<N> parentType = typeResolver.apply(parent);
        if (parentType != null) parentType.addChild(parent, child);
    ...
    private void openDialogForSelected() {
        ...
        if (ed == null || !ed.supportsDialog()) return;
