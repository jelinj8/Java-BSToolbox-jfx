 ...
 *
 * <p>Each item is displayed as text and edited inline on double-click or ENTER.
 * The editor type is supplied via {@link IValueEditorProvider}. The {@link #getItems() items}
 * list is a live {@link ObservableList} that updates as the user edits.
 */
public class ListEditor<V> extends VBox {
    ...
    // No extractor — extractor-triggered UPDATE events cause TableViewSkin to cancel
    // the active edit. Instead, items is kept in sync via direct per-entry value listeners.
    ...
    public ListEditor(IValueEditorProvider<V> provider) {
        ...
        table.addEventFilter(KeyEvent.KEY_PRESSED, e -> {
            if (e.getCode() == KeyCode.ENTER) {
                ...
                if (e.getCode() == KeyCode.INSERT) { e.consume(); addBtn.fire(); }
                else if (e.getCode() == KeyCode.DELETE) { e.consume(); delBtn.fire(); }
            }
        });
        ...
        addBtn.setOnAction(e -> {
            ...
            javafx.application.Platform.runLater(() ->
                    table.edit(entries.indexOf(entry), valCol));
        });
    ...
    public void setSelectedItem(V item) {
        entries.stream()
                .filter(e -> Objects.equals(e.value.get(), item))
                .findFirst()
