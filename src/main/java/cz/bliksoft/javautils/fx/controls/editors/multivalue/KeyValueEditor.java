 ...
 *
 * <p>When a {@link #propertyRegistryProperty() propertyRegistry} is set, the key column
 * restricts selection to the defined property names using a codebook popup. The value
 * column editor is resolved per key type via {@link cz.bliksoft.javautils.fx.controls.editors.ValueEditorFactory}.
 *
 * <p>The {@link #getValues() values} map is a live {@link ObservableMap} that updates
 * as the user edits, suitable for binding validation state.
 */
public class KeyValueEditor<V> extends VBox {
    ...
    private final ObservableList<KVEntry<V>> entries = FXCollections.observableArrayList(
            entry -> new Observable[]{ entry.key, entry.value });
    ...
    public KeyValueEditor(IValueEditorProvider<V> defaultProvider) {
        ...
        // valCol must be declared first so the keyCol cell-factory lambda can capture it.
