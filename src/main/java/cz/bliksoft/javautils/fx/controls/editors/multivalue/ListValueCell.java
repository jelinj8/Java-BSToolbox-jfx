 ...
 *
 * <p>Shows a text label in display mode. Double-click or ENTER starts inline edit.
 * When the provider supports a dialog, a "…" button is shown in both display and edit mode.
 * The editor node and graphics are created once and reused for all rows the cell is recycled to.
 */
final class ListValueCell<V> extends TableCell<ListEntry<V>, V> {
    ...
    protected void updateItem(V item, boolean empty) {
        ...
        // The extractor on `entries` fires an UPDATE event on every keypress (value change),
        // which would otherwise tear down and re-establish the bidirectional binding mid-edit.
        if (isEditing()) return;
